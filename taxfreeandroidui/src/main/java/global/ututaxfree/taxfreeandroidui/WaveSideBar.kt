...
class WaveSideBar @JvmOverloads constructor(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val areaLeft: Float =
            if (mSideBarPosition == POSITION_LEFT) 0F else width - mBarWidth - paddingRight
        val areaRight =
            if (mSideBarPosition == POSITION_LEFT) paddingLeft + areaLeft + mBarWidth else width.toFloat()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        while (i < mIndexItemsLength) {
            ...
            if (mSideBarPosition == POSITION_LEFT) {
                when (mTextAlignment) {
                    TEXT_ALIGN_CENTER -> baseLineX =
                        paddingLeft + mBarWidth / 2 + mMaxOffset * scale
                    TEXT_ALIGN_LEFT -> baseLineX =
                        paddingLeft + mMaxOffset * scale
                    TEXT_ALIGN_RIGHT -> baseLineX =
                        paddingLeft + mBarWidth + mMaxOffset * scale
                }
            } else {
                when (mTextAlignment) {
                    TEXT_ALIGN_CENTER -> baseLineX =
                        width - paddingRight - mBarWidth / 2 - mMaxOffset * scale
                    TEXT_ALIGN_RIGHT -> baseLineX =
                        width - paddingRight - mMaxOffset * scale
                    TEXT_ALIGN_LEFT -> baseLineX =
                        width - paddingRight - mBarWidth - mMaxOffset * scale
                }
            }
            // draw
            canvas.drawText(
    ...
    private fun getItemScale(index: Int): Float {
        ...
        if (mCurrentIndex != -1) {
            val distance =
                Math.abs(mCurrentY - (mIndexItemHeight * index + mIndexItemHeight / 2)) / mIndexItemHeight
    ...
    init {
        ...
        mTextColor = typedArray.getColor(R.styleable.WaveSideBar_sidebar_text_color, 666666)
        mTextSize = typedArray.getDimension(
            R.styleable.WaveSideBar_sidebar_text_size,
            sp2px(DEFAULT_TEXT_SIZE)
        )
        mMaxOffset = typedArray.getDimension(
            R.styleable.WaveSideBar_sidebar_max_offset,
            dp2px(DEFAULT_MAX_OFFSET)
        )
        mSideBarPosition = typedArray.getInt(
            R.styleable.WaveSideBar_sidebar_position,
            POSITION_RIGHT
        )
        mTextAlignment = typedArray.getInt(
            R.styleable.WaveSideBar_sidebar_text_alignment,
            TEXT_ALIGN_CENTER
        )
        typedArray.recycle()
